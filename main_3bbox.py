...
def predict_step_n(history, scaler, model, step=3, device="cpu"):
    """ Dự báo bbox cho n bước tiếp theo (step >= 1). """
    hist = np.array(history)
    for _ in range(step):
        input_seq = multi_level_lag_scheme(hist)
        ...
        bbox_pred = denormalize_output(pred, scaler)
        hist = np.vstack([hist[1:], pred])
    return bbox_pred[:4].astype(int)
...
def main(video_path):
    ...
    FUTURE_STEP = 3  # vẽ bbox vàng cho bước t+3
    ...
    while cap.isOpened():
        ...
        for det in detections:
            ...
            for tracker in trackers:
                dist = np.linalg.norm(np.array(get_center(tracker.bbox)) - np.array(get_center(det["box"])))
                if dist < min_dist:
                    min_dist = dist
        ...
        for tracker in trackers:
            ...
            # Predict bbox t+1 (màu đỏ) & lưu lại để so sánh MAE/IoU
            if len(tracker.history) == 7:
                ...
                # === VẼ CHỈ 1 BBOX VÀNG CHO t+FUTURE_STEP ===
                pred_box_future = predict_step_n(tracker.history, scaler, lstm_model, step=FUTURE_STEP, device=DEVICE)
                draw_dashed_rectangle(
                    frame, pred_box_future, color=(0, 255, 255), thickness=2, dash_length=8
                )
                cv2.putText(
                    frame, f"ID {tracker.id} t+{FUTURE_STEP}",
                    (pred_box_future[0], pred_box_future[1]-8),
        ...
        cv2.imshow("YOLO+Kalman+LSTM (Green: Real, Red: t+1 Predict, Yellow dashed: t+3 Future)", frame)
