...
def read_all_csv(root: str = "data/finetune") -> pd.DataFrame:
    ...
    for f in files:
        ...
        dfs.append(df)
    if not dfs:
        raise RuntimeError("No CSV found under data/finetune")
...
def compute_outlier_mask(df: pd.DataFrame) -> pd.Series:
    ...
    return cond_bounds | cond_area_jump.fillna(False)


def main():
    df = read_all_csv()
    print(f"Loaded {len(df)} rows from finetune CSVs")
    ...
    merged_path = os.path.join(out_dir, "finetune_merged_clean.csv")
