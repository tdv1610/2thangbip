...
from project.models.predictor import LSTMSeq2SeqPredictor  # Sử dụng model mới seq2seq
...
# ==== Load data ====
X_train = np.load(os.path.join(ARRAYS_DIR, "X_train.npy"))    # (num_samples, 7, 9)
y_train = np.load(os.path.join(ARRAYS_DIR, "y_train.npy"))    # (num_samples, 10, 9)
...
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    if epoch_loss < best_loss:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), os.path.join(WEIGHTS_DIR, "bbox_predictor.pth"))
    else:
        patience_counter += 1
...
np.save(os.path.join(ARRAYS_DIR, "train_loss_history.npy"), np.array(loss_history))
